...
namespace As.Applications.IO
{
    ...
    {
        static readonly ILogger Log = (ILogger)LogManager.GetLog(typeof(XmlStream));
            ...
            where T : class, new()
        {
            try
            {
                ...
                {
                    result = new();
                    return Write(path, result, create_backup: false, noexcept: false);
                }
                result = v!;
            ...
            catch (Exception x)
            {
                if (noexcept)
                {
                    Log.ErrorFormat($"ReadOf{nameof(T)}: fail; path = \"{path}\"", x);
                    result = default!;
                    return false;
                }
                throw;
